...
public class ControlCircleTraceTargetPaced : MonoBehaviour
{
    // States
    ...
    // Special condition: subject finishes lap ahead of pacer
    private bool subjectHasFinishedLapAheadOfPacerFlag = false;
    private float tempPacerDesiredLapTimeInSeconds; // we need to temporarily store the desired lap time, passed in by the level manager, in this special case.
    ...
    void Update()
    {
        ...
        {
                case settingUpState:
                bool levelManagerSetupComplete = levelManagerScript.IsLevelManagerSetupComplete();
                if (levelManagerSetupComplete == true)
                {
                    // Now that we have the base of support established from the Vicon data,
                    // get the perimeters corresponding to the query thetas vector
                    GetPerimeterAtThetaQueryPointsMeasuredFromThetaZeroViconFrame(indexingThetasInViconFrame);

                    // Sanity check: both the theta query points vector and corresponding perimeters covered vector should have the same length.
                    Debug.Log("After initializing theta query points and perimeter vectors, they have length: "
                        + indexingThetasInViconFrame.Length + ", " + thetaCorrespondingPerimetersMeasuredFromThetaZeroViconFrame.Length);

                    // Switch to the pushable-by-player state
                    changeActiveState(pushedByPlayerState);
                }
                break;
            ...
            case pacerIdlingState:
                // If we have entered the pacer idling state after completing a lap, but the subject finished the lap ahead of the pacer, 
                // then immediately transition back to the pacingState. 
                if(subjectHasFinishedLapAheadOfPacerFlag == true)
                {
                    ComputeLapParametersAndReenterPacingState(tempPacerDesiredLapTimeInSeconds); // will exit the pacer idling state and re-enter pacingState.
                }

                // NOTE: the typical "exit criteria" for this state is when the subject finishes the lap. The level manager 
                // calls a public funcion in this script to exit the Idling state.
                break;
        }
    }
    ...
    // Note that the level manager calls this function when the subject (NOT the pacer) has 
    // finished a lap. 
    // If the pacer is still completing a lap, don't reset yet, just set a flag to reset later.
    public void StartPacerLap(float pacerDesiredLapTimeInSeconds)
    {
        ...
        if(currentState == pushedByPlayerState || currentState == pacerIdlingState)
        {
            // Call the function that triggers the pacer for the next lap: set desired pacer linear velocity based on lap time, re-enter the pacing state. 
            ComputeLapParametersAndReenterPacingState(pacerDesiredLapTimeInSeconds);
        }else if(currentState == pacingState) // If we're still completing the last lap
        {
            // Set a flag indicating that the subject has completed the lap, so when the pacer completes its lap, 
            // we will briefly enter pacerIdling state but then immediately transition back to pacingState.
            subjectHasFinishedLapAheadOfPacerFlag = true;
            // Temporarily store the pacerDesiredLapTimeInSeconds parameter so that we can pass it to the 
            // ComputeLapParametersAndReenterPacingState() function when the pacer finishes the lap.
            tempPacerDesiredLapTimeInSeconds = pacerDesiredLapTimeInSeconds;
        }
    }
    ...
    private void ComputeLapParametersAndReenterPacingState(float pacerDesiredLapTimeInSeconds)
    {
    ...
    private void changeActiveState(string newState)
    {
        ...
        if(currentState == settingUpState)
        {
            exitSettingUpState();
        }
        else if (currentState == pushedByPlayerState)
        {
    ...
    private void exitSettingUpState()
    {
        // do nothing for now
    }
    ...
    private void enterPacingState()
    {
        ...
        // Reset the flag for keeping track of if the subject finishes the lap before the pacer finishes the lap. 
        subjectHasFinishedLapAheadOfPacerFlag = false;
