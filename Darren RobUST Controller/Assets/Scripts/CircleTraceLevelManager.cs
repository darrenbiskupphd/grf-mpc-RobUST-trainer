...
public class CircleTraceLevelManager : LevelManagerScriptAbstractClass
{
    ...
    private ControlCircleTraceTarget circleTraceTargetControlScript; // the script that controls the target on the circle
    ...
    // Task  name
    private const string thisTaskNameString = "CircleTrace";
    ...
    private bool traceTargetHasPassedZeroOnItsTripAroundTheCircle = false;
    ...
    private float tracingTrajectoryPercentOfScreenToFill = 0.6f; //used in mapping from Vicon to Unity frame
    ...
    void Start()
    {
        ...
        circleTraceTargetControlScript = circleTraceTarget.GetComponent<ControlCircleTraceTarget>();
    ...
    **************************/
    ...
    private uint getQuadrantFromAngle(float angleInRadians)
    {
    ...
    private (float, float) getEllipseHeightAndWidthBasedOnQuadrant(uint quadrant)
    {
    ...
    private void setCircleTraceTargetAngle(float desiredAngleToTargetInViconFrame)
    {
        ...
        // Set the current trace target position
        currentTraceTargetAngleInDegrees = convertAngleFromViconFrameToUnityFrameOrViceVersa(desiredAngleToTargetInViconFrame);
    ...
    // This function may be OLD - DELETE IF NOT CALLED!
    private void setTargetAngleInViconFrameBasedOnTargetAngleInUnityFrame()
    {
        if(rightwardsSign < 0 ) //if the coordinate axes are inverted between Vicon frame and Unity frame
        {
            currentTraceTargetAngleInDegreesViconFrame = currentTraceTargetAngleInDegrees + 180;
            if(currentTraceTargetAngleInDegreesViconFrame > 360.0f) //if the angle is greater than 360 degrees
            {
                // Then we subtract 360 degrees to keep the angle between 0 and 360
                currentTraceTargetAngleInDegreesViconFrame = currentTraceTargetAngleInDegreesViconFrame - 360.0f;
            }
        }
    }
    ...
    private void setFrameAndTrialDataNaming()
    {
        ...
        string subdirectoryString = "Subject" + subjectSpecificDataScript.getSubjectNumberString() + "/" + "CircleTrace" + "/" + dateString + "/";
    ...
    private void changeActiveState(string newState)
   {
    ...
    private void enterWaitingForHomeState()
    {
        ...
        // (if we're syncing with external hardware (e.g. Vicon link box, EMGs, etc.))
    ...
    private bool checkPlayerPositionAndUpdateTraceTarget()
    {
        ...
        if (!isPlayerAtHome) //if the player is far out enough
        {
            ...
            float newTargetAngle = getUpdatedDirectionSpecificTargetAngleToIndicator(currentPlayerAngleInDegreesViconFrame,
            ...
            trialCompleted = updateProgressAroundCircleBasedOnNewTargetAngle(newTargetAngle);

            // Set the new target angle
            setCircleTraceTargetAngle(newTargetAngle);
    ...
    float playerXRelativeToCenterViconFrame, float playerYRelativeToCenterViconFrame)
    {
        ...
        float degreesInCircle = 360.0f;
        if (currentTraceTargetAngleInDegreesViconFrameLocal < minimumAngularProximityOfPlayerToTargetDegrees) //if the target has a small positive value less than the 
                                                                                                              //minimumAngularProximityOfPlayerToTargetDegrees, add 360 to it first
        {
            //currentTraceTargetAngleInDegreesLocal = currentTraceTargetAngleInDegrees + 360;
...
/*    private float getUpdatedDirectionSpecificTargetAngleToIndicator(float currentPlayerAngleViconFrame, 
        float playerXRelativeToCenterViconFrame, float playerYRelativeToCenterViconFrame)
    {
        ...
        // Figure out if the player is "behind" relative to the target (e.g., if
        // travel is counterclockwise, then closer in the clockwise direction than in the ccw direction).
        bool playerBehindTarget = false;
        bool playerCloseEnoughToPushTarget = false;
        float newTargetAngle = currentTraceTargetAngleInDegreesViconFrameLocal; // set equal to current target angle for now, will be updated
                                                                                 // if a change occurs
        if (!currentlyTravelingClockwise) //if traveling counterclockwise
        {
            playerBehindTarget = (currentTraceTargetAngleInDegreesViconFrameLocal - currentPlayerAngleViconFrame >= 0)
                || ((currentTraceTargetAngleInDegreesViconFrameLocal - currentPlayerAngleViconFrame) <= -(degreesInCircle - minimumAngularProximityOfPlayerToTargetDegrees));

            //if the player is behind the target, see if it is close enough to "push" the target further around the circle
            if (playerBehindTarget)
            {
                Debug.Log("Vicon coordinates relative to center and angle of indicator (x,y, theta): ( " + (currentTraceIndicatorPositionViconFrame.x - centerOfBaseOfSupportXPosViconFrame)
                    + ", " + (currentTraceIndicatorPositionViconFrame.y - centerOfBaseOfSupportYPosViconFrame)
                    + ", " + currentTraceTargetAngleInDegreesViconFrameLocal);
                ...
                playerCloseEnoughToPushTarget = ((Mathf.Abs(currentTraceTargetAngleInDegreesViconFrameLocal - currentPlayerAngleViconFrame) < minimumAngularProximityOfPlayerToTargetDegrees)
                                                || (Mathf.Abs(currentTraceTargetAngleInDegreesViconFrameLocal - currentPlayerAngleViconFrame) >= (degreesInCircle - minimumAngularProximityOfPlayerToTargetDegrees)));

                if (playerCloseEnoughToPushTarget)
                {
                    Debug.Log("Push");
                    //move the target to a new angle, minimumAngularProximityOfPlayerToTargetDegrees ahead of the player
                    newTargetAngle = currentPlayerAngleViconFrame + minimumAngularProximityOfPlayerToTargetDegrees;
        ...
        else //if traveling clockwise
        {
            ...
            if (playerBehindTarget)
            {
                ...
                //if it is close enough to push, we move the target
                if (playerCloseEnoughToPushTarget)
                {
                    Debug.Log("Push");
                    //move the target to a new angle, minimumAngularProximityOfPlayerToTargetDegrees ahead of the player
    ...
    private bool updateProgressAroundCircleBasedOnNewTargetAngle(float newTargetAngleViconFrame)
    {
        ...
        if (Mathf.Abs(newTargetAngleViconFrame - currentTraceTargetAngleInDegreesViconFrame) > thresholdForRollingAroundCircle)
        {
            traceTargetHasPassedZeroOnItsTripAroundTheCircle = true;
        }
        ...
        if (!currentlyTravelingClockwise) // if traveling counterclockwise
        {
            ...
            if (traceTargetHasPassedZeroOnItsTripAroundTheCircle && (newTargetAngleViconFrame > circleTraceTargetStartAngleCounterclockwiseViconFrame))
            {
                //then we have completed a full revolution. Note this by changing the flag.
                trialCompleted = true;

                //reset the target has passed zero flag
                traceTargetHasPassedZeroOnItsTripAroundTheCircle = false;
            }
        }
        else // if traveling clockwise
        {
            //if the trace target has passed zero on this trial and is smaller than the starting angle
            if (traceTargetHasPassedZeroOnItsTripAroundTheCircle && (newTargetAngleViconFrame < circleTraceTargetStartAngleClockwiseViconFrame))
            {
                //then we have completed a full revolution. Note this by changing the flag.
                trialCompleted = true;
